...
# === CẤU HÌNH TELEGRAM ===
BOT_TOKEN = "8076967422:AAH_kBAtW-IKHaHj0ekBUm5x2OEdxKNLR5w"
...
def send_prediction_update(session):
    ...
    message = (
        ...
        f"══════════════════════════\n"
        f"*✨ Cre: Trường Đông & Vinh Máy Cày ✨*"
    )
...
def background_task():
    ws_url = "wss://nhicuto.fun/game_sunwin/ws?id=Cskhtool11&key=NhiCuTo"
    reconnect_delay = 5
    while True:
        try:
                ...
                if "message" in update:
                    ...
                    if text:
                        if text.startswith("/start"):
                            ...
                            buttons = [
                                ...
                                [{"text": f"{EMOJI['money_bag']} Liên hệ mua key", "url": "https://t.me/vinhmaycayuytins1"}]
                            ]
                                ...
                                if len(parts) >= 2:
                                    ...
                                    if time_str and time_str != "vĩnh viễn" and time_str != "unlimited":
                                        ...
                                        if len(time_parts) >= 2 and time_parts[0].isdigit():
                                            ...
                                            else:send_telegram(chat_id, f"{EMOJI['warning']} Đơn vị thời gian không hợp lệ. Ví dụ: '30 ngày', '1 tuần', '6 tháng', '1 năm'.")
                                                continue
                                        else:
                    ...
                    if callback_data.startswith("broadcast_confirm:"):
                        ...
                        help_activate_message = (
                            ...
                            f"{EMOJI['team']} Liên hệ: @vinhmaycayuytins1 để được hỗ trợ"
                        )
...
def main():
    ...
    if c.fetchone()[0] == 0:
        print(f"{EMOJI['admin']} Thêm admin đầu tiên với ID: 7133191568 và 6381480476")
        c.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (7133191568,))
