...
# === CẤU HÌNH TELEGRAM ===
BOT_TOKEN = "8178870488:AAEEBrlIbzIhdq2mJDkGfKVYzY8UbmlrsoY"
...
def background_task():
    ws_url = "ws://163.61.110.10:8000/game_sunwin/ws?id=Cstooldudoan11&key=duyduy2212"
    reconnect_delay = 5
    while True:
        try:
                ...
                if "message" in update:
                    ...
                    if callback_data.startswith("broadcast_confirm:"):
                        ...
                        help_activate_message = (
                            ...
                            f"{EMOJI['team']} Liên hệ: @Cstooldudoan11 để được hỗ trợ"
                        )
...
def main():
    ...
    if c.fetchone()[0] == 0:
        print(f"{EMOJI['admin']} Thêm admin đầu tiên với ID: 6381480476 và 6906617636")
        c.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (6381480476,))
        c.execute("INSERT OR IGNORE INTO admins (chat_id) VALUES (?)", (6906617636,))
        conn.commit()
    conn.close()
